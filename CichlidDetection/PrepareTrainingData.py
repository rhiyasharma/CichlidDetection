...
def initialising(pid):
    ...
    image_data_path = '__AnnotatedData/BoxedFish/BoxedImages/{}.tar'.format(pid)
    remoteDir = cloudMasterDir + '{}'.format(pid)
    ...
    if 'videoCropPoints.npy' and 'videoCrop.npy' in remote_files.stdout.split():
        video_points_data_path = '{}/videoCropPoints.npy'.format(pid)
        video_crop_data_path = '{}/videoCrop.npy'.format(pid)
    else:
        video_points_data_path = '{}/MasterAnalysisFiles/VideoPoints.npy'.format(pid)
        video_crop_data_path = '{}/MasterAnalysisFiles/VideoCrop.npy'.format(pid)
...
def download(pid, cloudMasterDir, annotated_data, image_data, video_points_file, video_crop_file, localDir, imgfile_name):
    ...
    if file1 not in files:
        ...
        downloaded_file_1 = subprocess.run(['rclone', 'copy', cloudMasterDir + annotated_data, localDir],
                                           stderr=subprocess.PIPE)

    if file2 not in files:
        print('Downloading images')
        # Download images
        downloaded_file_2 = subprocess.run(['rclone', 'copy', cloudMasterDir + image_data, localDir],
                                           stderr=subprocess.PIPE)

    if file3 not in files:
        print('Downloading video points')
        # Download video points file
        downloaded_file_3 = subprocess.run(['rclone', 'copy', cloudMasterDir + video_points_file, localDir],
                                           stderr=subprocess.PIPE)

    if file4 not in files:
        print('Downloading video crop')
        # Download video points file
        downloaded_file_4 = subprocess.run(['rclone', 'copy', cloudMasterDir + video_crop_file, localDir],
                                           stderr=subprocess.PIPE)
...
def prep_data(pid, view=False):
    ...
    cloud_master_directory, annotated_data_path, image_file_path, video_points_file_path, video_crop_file_path, local_directory = initialising(pid)
    localDir, ann_file, img_file, vp_file, vc_file = download(pid, cloud_master_directory, annotated_data_path,
                                                              image_file_path, video_points_file_path,
                                                              video_crop_file_path, local_directory, imgfile_name)
